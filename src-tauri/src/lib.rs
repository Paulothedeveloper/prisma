...
/// Batch Watermark (plugin do Eagle): salva a imagem com marca d'água (PNG) ao lado do original
/// com sufixo "_marcadagua" e cataloga. Não-destrutivo.
#[tauri::command]
fn save_watermarked(app: tauri::AppHandle, near_path: String, data: Vec<u8>) -> Result<String, String> {
    let state = app.state::<AppState>();
    let p = std::path::PathBuf::from(&near_path);
    let parent = p.parent().map(|d| d.to_path_buf()).unwrap_or_default();
    let stem = p.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default();
    ...
    let out = loop {
        let suffix = if n == 1 { String::new() } else { format!(" {n}") };
        let cand = parent.join(format!("{stem}_marcadagua{suffix}.png"));
        if !cand.exists() {
            break cand;
        }
        n += 1;
...
#[tauri::command]
fn set_favorite_many(app: tauri::AppHandle, ids: Vec<i64>, fav: bool) -> Result<(), String> {
    let state = app.state::<AppState>();
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    db::set_favorite_many(&conn, &ids, fav).map_err(|e| e.to_string())
}
...
fn tag_many(
    ...
    let state = app.state::<AppState>();
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    db::tag_many(&conn, &ids, name.trim(), color.as_deref()).map_err(|e| e.to_string())
}
